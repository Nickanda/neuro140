...
class Agent:
        ...
        if load_model:
            try:
                ...
                self.model.load_weights(
                    "././Deep Q-Learning - PACMAN/final_version.weights.h5"
                )  # Use a new file name
    ...
    def train_agent(self):
        ...
        for index in range(self.batch_size):
            if not done[index]:
                current_q_values[index][action[index]] = reward[index] + self.gamma * (
                    np.amax(future_q_values[index])
                )
            else:
        ...
        for episode in range(num_episodes):
            ...
            if episode % 25 == 0:
                agent.model.save_weights(
                    "./Deep Q-Learning - PACMAN/trial_v1_06.weights.h5"
                )
